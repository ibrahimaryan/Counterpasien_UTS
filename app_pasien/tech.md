...
const String AUTH_EMAIL = 'perawat@rumahsakit.com';
const String AUTH_PASSWORD = 'passwordaman';
...
void main() async {
  ...
  await Firebase.initializeApp(); // Menganggap inisialisasi default
...
class _PatientScreenState extends State<PatientScreen> {
  ...
  Future<void> _toggleStatus(int patientNumber, int currentStatus) async {
    ...
    if (currentlySafe) {
      ...
      // Menggunakan runTransaction untuk memastikan kenaikan yang aman (atomic)
      final transactionResult = await counterRef.runTransaction((MutableData mutableData) async {
        int? currentValue = mutableData.value as int?;
        if (currentValue == null) {
          mutableData.value = 1; // Inisialisasi jika belum ada
        } else {
          mutableData.value = currentValue + 1; // Naikkan
        }
        return mutableData;
      });

      if (transactionResult.error != null) {
        print('Gagal menjalankan transaksi counter: ${transactionResult.error!.message}');
      } else {
        print('Counter berhasil dinaikkan. Nilai baru: ${transactionResult.snapshot.value}');
      }
    }
  }
    ...
    return Padding(
      ...
      child: ElevatedButton(
        style: ElevatedButton.styleFrom(
          backgroundColor: needsHelp ? Colors.red.shade700 : Colors.green.shade600,
